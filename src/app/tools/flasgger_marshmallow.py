...
def swagger_decorator(
        ...
        headers_schema=None, response_schema=None, blueprint_name = 'default'
):
    def decorator(func):
        ...
        def wrapper(*args, **kw):
            ...
            new_query = {}
            if type(query_params) is not str:
                for param in query_params:
                    if '[]' in param:
                        ...
                        for elem in param_list:
                            good_elem = json.loads(elem)
                            new_list.append(good_elem)
                        new_query[param_name] = new_list
                    else:
                        try:
                            new_query[param] = json.loads(query_params[param])
                        except:
                            new_query[param] = query_params[param]

            else:
                new_query = json.loads(query_params)
            ...
            try:
                ...
                query_schema and setattr(request, 'query_schema', query_schema().load(new_query or {}))
